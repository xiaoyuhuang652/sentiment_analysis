# transformer_encoder_text_classification.py
...
# =====================
# 6. Transformer Encoder Block (Pre-LN)
# =====================
class TransformerEncoderBlock(nn.Module):
    def __init__(self, embed_dim=128, num_heads=8, dropout=0.1, ffn_expansion=4):
        super().__init__()
...
# =====================
# 8. Standard Transformer Encoder Classifier
# =====================
class TransformerClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim=128, num_layers=4, num_heads=8, num_classes=2, dropout=0.1, max_len=500):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=0)
        self.cls_token = nn.Parameter(torch.randn(1,1,embed_dim))
        self.pos_embedding = PositionEmbedding(max_len+1, embed_dim)  # +1 for CLS token
...
for epoch in range(10):
    ...
    if acc > best_acc:
        best_acc = acc
        counter = 0
        torch.save(model.state_dict(), "results/best_transformer.pth")
...
plt.legend()
plt.title("Standard Transformer Training Curve")
plt.savefig("results/Standard Transformer_training_curve.png")
plt.show()
