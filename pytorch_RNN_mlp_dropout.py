...
random.seed(42)
train_sample = random.sample(list(zip(train_texts, train_labels)), 10000)
test_sample  = random.sample(list(zip(test_texts, test_labels)), 2000)
...
# =====================
# 5. 模型定义 (RNN + Dropout + MLP)
# =====================
class RNNMLPDropout(nn.Module):
    def __init__(self, vocab_size, embed_dim=256, hidden_dim=256, dropout=0.3):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=0)
       #self.rnn = nn.RNN(embed_dim, hidden_dim, batch_first=True)
        self.rnn = nn.RNN(embed_dim, hidden_dim, num_layers=3, batch_first=True, dropout=0.3)
    ...
    def forward(self, x):
        ...
        final = hidden[-1]        # [batch, hidden_dim]
...
model = RNNMLPDropout(vocab_size=vocab_size).to(device)
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for X_batch, y_batch in val_loader:
            ...
            val_labels_list.extend(y_batch.cpu().tolist())

    val_acc = accuracy_score(val_labels_list, val_preds)
    if val_acc > best_val_acc:
        best_val_acc = val_acc
        counter = 0
        torch.save(model.state_dict(), "results/best_rnn_dropout_model.pth")
...
with torch.no_grad():
    for X_batch, y_batch in test_loader:
        ...
        test_labels_list.extend(y_batch.cpu().tolist())
...
plt.xlabel("Epoch")
plt.ylabel("Value")
plt.title("Training Loss & Validation Accuracy (RNN MLP + dropout)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("results/training_curves_RNN_dropout.png")
plt.show()